...
function SourcePage({ type }) {
  ...
  const accessLabel = access => access === 'public_listing' ? '公告公开·业务受限' : '需登录/会员';
  return <div className="source-page"><p>{authentication ? '经逐一实测审查后分两类：① 公告可匿名浏览、但下载/投标等业务操作需登录（公告公开·业务受限）；② 公告内容本身需登录、SPA 动态加载或 WAF 反爬，匿名无法稳定浏览（需登录/会员）。系统不会绕过鉴权；公告可浏览的直接抓，需登录的由你在已授权会话中抓取。' : '仅保留公告完全公开可匿名浏览的入口。公开浏览不代表可以绕过下载、投标或付费权限；正式入账仍须保存原公告。'}</p><p className="source-count">已收录 {entries.length} 个{authentication ? '受限' : '匿名公开'}平台。</p><table><thead><tr><th>平台</th>{authentication && <th>权限</th>}<th>覆盖内容</th><th>复审说明</th></tr></thead><tbody>{entries.map(platform => <tr key={platform.id}><td><a href={platform.entryUrl} target="_blank" rel="noreferrer">{platform.name} ↗</a></td>{authentication && <td><span className="access-tag">{accessLabel(platform.access)}</span></td>}<td>{platform.coverage}</td><td>{platform.auditNote}</td></tr>)}</tbody></table></div>;
}
